...
var createAndSavePerson = function(done) {
  var kshitiz=new Person({name:'Kshitiz',age:24,favoriteFoods:["Bread","Cake","Pizza"]});
  
   kshitiz.save(function(err, data) {
...
// 'arrayOfPeople'.
var arrayOfPeople=[{name:"Rahul",age:25,favoriteFoods:["Cake","Bread","Rice"]},
                   {name:"Sandeep",age:25,favoriteFoods:["Idli","Bread","Rice"]}];
...
var findEditThenSave = function(personId, done) {
  ...
  Person.findById({_id: personId}, function (err, personFound) {
    ...
    personFound.save((err)=>{if(err,personFound)console.log(err);
                             done(null, personFound);});
});
};
...
var findAndUpdate = function(personName, done) {
  ...
  Person.findOneAndUpdate({name:personName},{$set:{age:20}},{ new: true },function(err,p){
...
var removeById = function(personId, done) {
  Person.findOneAndRemove({_id:personId},(err,p)=>{
    if(err) console.log(err);
    done(null ,p);
  });
...
var removeManyPeople = function(done) {
  ...
  Person.remove({name:nameToRemove},(err,p)=>{
    if(err) console.log(err);
    done(null,p);
  });
};
